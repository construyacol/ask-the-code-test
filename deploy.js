...
const validateType = function (deployType) {
  if (!deployType || !deployType['bucket']) {
    console.error(`Deploy type not recognized ${deployType}. Use one of ${setupEntries.join(' | ')}`)
    process.exit()
  }
}
...
if (argType && argProfile) {
  ...
  console.log('You could run this script providing type and profile, like `yarn deploy staging s3_profile`')
  ...
  s3.listObjects({Bucket: bucket}, function(err2, data) {
    if (err2) {
      ...
      if (data.Contents.length > 0) {
        console.log('Deleting', data.Contents.map(file => file.Key))

        var s3delete = {
          Bucket: bucket,
          Delete: {
            Objects: data.Contents.map(file => {return {Key: file.Key}})
          }
        }
