...
const TicketContainer = loadable(() =>
  import("../widgets/ticket/ticketContainer")
);
const PairList = loadable(() => import("../wallets/views/swap_pair_list"));
const ConfirmationModal = loadable(() =>
  import("../widgets/modal/confirmation")
);
const Kyc = loadable(() => import("../kyc/kyc_container"));
const DepositContainer = loadable(() =>
  import("../wallets/deposit/depositContainer")
);
const WithdrawAccountForm = loadable(() =>
  import("../withdrawAccounts/new/withdrawAccountForm")
);
const NewWallet = loadable(() =>
  import("../wallets/newWallet/newWalletContainer")
);
      ...
      <ModalContainer condition={isModalVisible}>
        <ModalLayout modalView={modalView} loader={loader}>
          ...
          <Route
            exact
            strict
            path="/withdraw_accounts/activity/:account_id/:tx_path/:order_id"
            component={TicketContainer}
          />
          <Route
            strict
            path="/wallets/deposit/:account_id"
            render={(renderProps) => {
              return (
                <DepositContainer
                  {...renderProps}
          ...
          <Route
            exact
            path="/security"
            component={current === "2auth" ? TwoFactorActivate : Kyc}
          />
        </ModalLayout>
      </ModalContainer>
