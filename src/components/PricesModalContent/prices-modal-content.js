...
const compare = (a, b) => {
  if (a.buy_pair.includes(FIRST_CRITERIA))
    return 1;
  if (a.buy_pair > b.buy_pair)
    return 1;
  if (b.buy_pair > a.buy_pair)
    return -1;
  return 0;
}

const PairsSelect = ({ pairs, currentPair, changePair, selectRef, setShowSelect }) => {
  return (
      ...
      {pairs.sort(compare).map((pair, key) => {
        ...
        >
          ...
          <img
            ...
            src={require(`../../assets/prices-modal/${rule ? 'ic_select' : 'ic_arrow_right_desabled'}.svg`)}
            alt="" />
        </div>
      })
      }
