...
const MenuPrincipalLayout = (props) => {
  ...
  const {show_menu_principal, close_menu_principal, verification_state, openSelectCountry, navigateTo} = props

  const [acronym,
    setAcronym] = useState()
  const actions = useActions()
  const idForLogoutButton = useKeyActionAsClick(true, 'logout-button', 27, true, 'onkeydown')
    ...
    <section
      ...
      <div className="menuItems">
        ...
        <section className={`section2 movil`}>
          <div id={idForLogoutButton} className="menuMovilItems close" onClick={logOut}>
            <p className="menuMovilItemTexts close fuente">{logoutButtonText}
