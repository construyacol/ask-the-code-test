...
function ContentTab(props) {
  ...
  const [menuItems, setMenuItems] = useState(
    navigation_components.wallets.items_menu
  );
  ...
  useEffect(() => {
    if (menuItems && pathname) {
      ...
      const next =
        menuItems[currentIndex + 1] && menuItems[currentIndex + 1].link;
      const prev =
        menuItems[currentIndex - 1] && menuItems[currentIndex - 1].link;
      navState.current = {
        current,
  ...
  useEffect(() => {
    ...
    const condition =
      primary_path === "wallets" ? haveBalances && menuItems : menuItems;

    setHaveMenu(condition ? menuItems.length > 0 : false);
  }, [current_wallet, menuItems, primary_path, wallets]);

  const getLink = (link) => {
    return `/${primary_path}/${link}/${current_wallet}${
      link === "activity" ? `/${params.currentFilter}` : ""
    }`;
  };
  ...
  const backButtonId = useKeyActionAsClick(
    true,
    "back-button-content-tab",
    8,
    true,
    "onkeyup"
  );
  const idNext = useKeyActionAsClick(
    true,
    "id-next-button",
    39,
    haveMenu,
  ...
  const idPrev = useKeyActionAsClick(
    true,
    "id-prev-button",
    37,
    haveMenu,
