...
export class ItemWallet extends Component {
  ...
  wallet_detail = async () => {
    ...
    if (this.props.current === "deposit") {
      ...
      return this.props.history.push(
        `/wallets/deposit/${this.props.wallet.id}`
      );
    }
    this.update_deposit_provider();
  ...
  delete_this_account = async () => {
    ...
    await this.props.coinsendaServices.deleteAccount(
      this.props.wallet.id,
      type === "withdraw" ? "withdraw" : "wallet"
    );
  ...
  depositar = async () => {
    if (this.props.wallet.currency_type === "fiat") {
      ...
      let verified = await this.props.coinsendaServices.getUserVerificationStatus(
        "level_1"
      );
      ...
      return this.props.action.current_section_params({
        current_wallet: this.state.current_wallet,
        deposit_direct_access: true,
      });
    }
  ...
  render() {
    ...
    return (
      <Fragment>
        <div
          className={` contWalleins ${clases} ${
            account_state === "deleting" && id_trigger
              ? "WIwalletDeleting"
              : account_state === "deleted" && id_trigger
              ? "WIwalletDeleted"
              : ""
          }`}
        >
          <div
            id="ItemWallet"
            className={` ${
              account_state === "deleted" && id_trigger ? "WIitemDeleted" : ""
            } ${current === "deposit" ? "ItemWallet2" : "ItemWallet"} ${
              type !== "withdraw"
                ? currency.currency
                : type === "withdraw" && !inscribed
                ? "NoInscribed"
                : "cop"
            } ${type !== "withdraw" ? "cryptoWallet" : ""}`}
            // className={`ItemWallet ${currency.currency}`}
          >
            <div
              className={`ItemWCta ${
                current_view === "detail" || type === "withdraw"
                  ? "noVisible"
                  : ""
              }`}
              onClick={this.wallet_detail}
            ></div>

            <div
              className={`ItemBarra ${type} ${
                current_view === "detail" ? "noVisible" : ""
              }`}
            >
              <div
                className={`ItemBarraI ${
                  type === "withdraw" ? "noVisible" : ""
                }`}
              >
                <i
                  className="far fa-arrow-alt-circle-up IRetiro IdeleteButton tooltip"
              ...
              <div className="ItemBarraI retweetCont">
                <i
                  className="fas fa-trash-alt IdeleteButton tooltip"
            ...
            <h1 className="IWText titu fuente tobe_continue">
            ...
            {type === "withdraw" ? (
              <Fragment>
                ...
                <p
                  ...
                  style={{ display: used_counter < 1 ? "none" : "flex" }}
                >
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withCoinsendaServices(ItemWallet))
);
