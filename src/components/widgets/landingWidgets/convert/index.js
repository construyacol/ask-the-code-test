  ...
  const handleChange = async (e) => {
    ...
    switch (e.target.name) {
      case "wantSpend":
        ...
        convertResult = await convertCurrencies(
          props.currentPair.primary_currency,
          wantSpendFormat,
          props.currentPair.id
        );
        setWantSpend(wantSpendFormat);
        setRecieved(
          convertResult.want_to_spend === "NaN"
            ? 0
            : convertResult.want_to_spend
        );
        break;
      case "recieved":
        return false;
  ...
  return (
    <div className="InputFormConverter">
      <InputFormConverter
        ...
        iconPosition="left"
        currency_short_name={
          props.currentPair && props.currentPair.primaryShortName
        }
      />

      <IconSwitch icon="swap" size={30} color="#0066AA" />
      ...
      <InputFormConverter
        ...
        value={recieved}
        icon={
          props.currentPair && props.currentPair.secondary_currency.currency
        }
        iconPosition="right"
        currency_short_name={
          props.currentPair && props.currentPair.secondaryShortName
        }
      />
    </div>
  );
