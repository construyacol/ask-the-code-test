    ...
    useEffect(() => {
      ...
      if(props.pairs.currentPair){
        const formatToCurrencyValue = async() => {
          const { buy_price, sell_price, secondary_currency } = props.pairs.currentPair
          let format_sell_price = await formatToCurrency(sell_price, secondary_currency, true)
          setSellPrice(format_sell_price)
          let format_buy_price = await formatToCurrency(buy_price, secondary_currency, true)
...

                {
                  ...
                  <div className="quoteMovil">
                    ...
                    <div className="controlQuoteMovil">
                      <p data-quote="buy" onClick={toggleQuoteMovil}  className={`controlQItem fuente ${quoteView === 'buy' ? 'active' : ''}`} >Te compramos a:</p>
                      <p data-quote="sell"  onClick={toggleQuoteMovil} className={`controlQItem fuente ${quoteView !== 'buy' ? 'active' : ''}`} >Te Vendemos a:</p>
                    </div>
                  </div>
                }
