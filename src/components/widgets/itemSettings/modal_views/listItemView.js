...
class ItemListView extends Component {
  ...
  render() {
    const { noIcon, item_active, item, theme, iconType } = this.props;

    const { id, code, name, flag } = item;
    ...
    return (
      <div
        id={`${id}`}
        className={`ItemlistViewart ${
          parseInt(item_active) === parseInt(id) ? "itemActiveListView" : ""
        } ${theme}`}
        ...
        <span id={`${id}`} className={`fuente span_text_itemView ${theme}`}>
          {
            ...
            parseInt(item_active) === parseInt(id) && (
              <img
                ...
                width="25"
                height="25"
                style={{ display: iconType ? "block" : "none" }}
              />
            )
          }
          {name}
            ...
            <div className="controlDespegableLIV">
              ...
              >
                <div className="contDesp">
                  <IconSwitch icon="arrow_right" size={25} color="#38ef7d" />
                </div>
              </div>
