...
class MVList extends Component {
  ...
  render() {
    const { noIcon, noFindbar, theme, iconType } = this.props;

    const { select_id, current_list, search, current_item } = this.state;
    ...
    return (
      <section
        className={`contListItemMV ${theme}`}
        style={{ paddingTop: noFindbar ? "10px" : "60px" }}
      >
        <div
          className={`findbar ${theme}`}
          style={{ display: noFindbar ? "none" : "grid" }}
        >
          <InputForm
            type="text"
            // name="account_name"
            ...
            placeholder={
              current_item ? current_item.name : "Ej. Cali, Medellin, Bogotá"
            }
          />
        </div>
        {search.length > 0
