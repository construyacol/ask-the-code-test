...
const ItemList = (props) => {
  ...
  const can_proceed = () => {
    const { deleting, current_order_loader } = props;

    if (deleting && current_order_loader) {
      return false;
    }
  };
  ...
  let statePendingSwap =
    socket_swap.state && socket_swap.id === id ? socket_swap.state : state;
  // let pendingSwap = unique_id === id && type_order === 'swap'
  let pendingSwap =
    type_order === "swap" &&
    (statePendingSwap === "pending" ||
      statePendingSwap === "processing" ||
  ...
  return (
    <div
      ref={element}
      className={`NDCont ${show && "shower"} ${
        statePendingSwap === "error" ? "errorSwap" : ""
      }     ${account_to ? "swap_bought" : ""} ${
        current_swap_done && swap_done_out
          ? "current_swap_done_out"
          : current_swap_done && swap_done_in
          ? "current_swap_done_in"
          : ""
      } ${
        lastPendingId === id && newDepositStyle
          ? "newDepositContainer"
          : deleting && current_order_loader === id
          ? "deletingWallet"
          : deleted && current_order_loader === id
          ? "walletDeleted"
          : "newDepositContainern"
      }`}
    >
      {show && (
        <Fragment>
          <div
            id={`${id}`}
            className={`ItemList fuente  ${
              current_swap_done && swap_done_out
                ? "current_swap_done_item_out"
                : current_swap_done && swap_done_in
                ? "current_swap_done_item_in"
                : ""
            }  ${
              pendingSwap && statePendingSwap === "processing"
                ? "swaProcessing"
                : pendingSwap && statePendingSwap === "done"
                ? "swapDone"
                : ""
            }   ${
              lastPendingId === id && newDepositStyle
          ...
          >
            ...
            {statePendingSwap !== "error" && (
              <div
                className={`ALDetail ${
                  loader && current_order_loader === id ? "InProcess" : ""
                }`}
                onClick={detail}
              ></div>
            )}
                ...
                <Fragment>
                  {type_order === "swap" && pendingSwap ? (
                    ...
                    <div className="ConfirmedTxT fuente2">
                      <p>
                        ...
                        <span>
                          /
                          {props.currencies &&
                            props.currencies[ticket.currency.currency]
                              .confirmations}
                        </span>
                      </p>
                    </div>
                    ...
                    <div className="fecha">
                      ...
                      >
                        {moment(ticket.created_at).format("DD")}
                      </p>
                      <p
                        className="ALmonthDate"
                        style={{
                          color:
                            currency_type === "fiat" &&
                            (state === "pending" || state === "confirmed")
                      ...
                      >
                        {moment(ticket.created_at).format("MMM").toUpperCase()}
                      </p>
                    </div>
                  )}
                </Fragment>
                ...
                <SwapAnimation
                  ...
                  colorIcon={
                    statePendingSwap === "done"
                      ...
                      : statePendingSwap === "processing"
                      ? "#77b59d"
                      : statePendingSwap === "pending" && "#ff8660"
                  }
                />
              ) : (
                <Fragment>
                  {state === "canceled" || statePendingSwap === "error" ? (
                    ...
                    <i
                      style={{
                        color:
                          state === "accepted" || statePendingSwap === "done"
                            ? "#1cb179"
                            : state === "pending"
                            ? "#ff8660"
                  ...
                  statePendingSwap === "pending"
                    ...
                    : type_order !== "swap" &&
                      (state === "rejected" ||
                        ...
                        statePendingSwap === "error")
                    ? "#f44336"
                    : type_order !== "swap" && state === "confirmed"
                    ? "#1cb179"
                    : "#80808029",
              }}
            >
                ...
                {!movil_viewport && (
                  <Fragment>
                    ...
                    type_order === "deposit" ? (
                      ...
                      "Error"
                    ) : state === "confirmed" && type_order === "withdraw" ? (
                      "Procesando"
                    ) : (
                      state
                    )}
              ...
              state === "pending" &&
                ...
                <PaymentConfirButton
                  clases={`ALconfirmButton ${
                    lastPendingId === id ? "ALbuttonActive" : "laReputas"
                  }`}
                  active={true}
                  type="primary"
                  siguiente={detail_payment}
                ...
                <Fragment>
                  ...
                  >
                    <div
                      className="fuente2 ALamount pentoni"
                      style={{
                        fontSize: "17px",
                      }}
                    >
                  ...
                  {!movil_viewport && (
                    <i
                      ...
                      style={{
                        color: state === "accepted" ? "#1cb179" : "gray",
                      }}
                    ></i>
                  )}
                </Fragment>
