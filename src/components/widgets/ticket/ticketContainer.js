...
class TicketContainer extends Component {
  ...
  get_deposit_payment_proof = async () => {
    ...
    if (ticket.type_order !== "deposit" || ticket.currency_type === "fiat") {
      return false;
    }
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (prevProps.current_ticket_state === this.props.current_ticket_state) {
      return false;
    }
  ...
  ticket_serve = async (ticket) => {
    ...
    if (type_order !== "swaps") {
      ...
      amount_neto = await formatToCurrency(
        ticket.amount_neto,
        ticket.currency,
        true
      );
      ...
      bought = await formatToCurrency(
        ticket.bought,
        ticket.currency_bought,
        true
      );
    }
    ...
    switch (type_order) {
      case "withdraws":
        if (!current_wallet || current_wallet.currency_type === "fiat") {
          let { wallets, withdraw_accounts, withdrawProviders } = this.props;
            ...
            current_ticket: [
              ...
              },
              {
                ...
                value: withdraw_account
                  ? withdraw_account.bank_name.ui_name
                  : "Cuenta eliminada",
                id: 2,
              ...
              },
              {
                ui_name: `${
                  withdraw_account
                    ? withdraw_account.account_number.ui_name
                    : "Cuenta eliminada"
                }:`,
                value:
                  withdraw_account && withdraw_account.account_number.value,
                id: 3,
              },
              {
                ui_name: `Ciudad:`,
              ...
              },
              {
                ...
                value: `${
                  withdraw_account ? withdraw_account.name : "Anonimo"
                } ${withdraw_account ? withdraw_account.surname : ""}`,
                id: 5,
              },
              {
          ...
          return this.setState({
            current_ticket: [
              ...
              {
                ui_name: `Enviado a:`,
                value: this.props.withdraw_accounts[ticket.withdraw_account_id]
                  .account_name.value,
                icon: ticket.currency.currency,
                id: 3,
              },
              ...
              },
              {
                ui_name: "Tx Id:",
                value:
                  ticket.state === "confirmed" ? "En proceso" : ticket.proof,
                type: ticket.state !== "confirmed" && "tx",
                copy: ticket.state !== "confirmed" && true,
                url_explorer:
                  ticket.state !== "confirmed" &&
                  currencies[ticket.currency.currency].node_url,
                id: 4,
              },
            ],
          });
      ...
      case "deposits":
        let { deposit_providers, currencies } = this.props;
        ...
        if (ticket.currency_type === "crypto") {
          return this.setState({
            current_ticket: [
              ...
              {
                ui_name: "Tx Id:",
                value: ticket.paymentProof
                  ? ticket.paymentProof.proof_of_payment.proof
                  : "Sin Registro",
                copy: ticket.paymentProof && true,
                url_explorer:
                  ticket.paymentProof &&
                  currencies[ticket.currency.currency] &&
                  currencies[ticket.currency.currency].node_url,
                id: 7,
              },
            ],
            ...
            current_ticket: [
              {
                ...
                // value:'kkk',
                value:
                  deposit_providers[ticket.deposit_provider_id].provider
                    .ui_name,
                // icon:'kkk',
                icon:
                  deposit_providers[ticket.deposit_provider_id].provider.name,
                id: 5,
              },
              {
                // ui_name:'dsdsd',
                ui_name:
                  deposit_providers[ticket.deposit_provider_id].provider.account
                    .account_id.ui_name,
                // value:'dsdsd',
                value:
                  deposit_providers[ticket.deposit_provider_id].provider.account
  ...
  render() {
    ...
    if (handleError) {
      return <ErrorView msg="Al parecer tu token ha caducado" />;
    }

    return (
      <section className="TicketContainer">
        <section
          className={`smartContainer ${step !== 1 ? "ticketStep2Border" : ""}`}
        >
          <div
            className={`ticketSlide`}
            style={{
...
function mapStateToProps(state, props) {
  ...
  const total_confirmations =
    current_wallet &&
    current_wallet.currency_type === "crypto" &&
    currency_list[ticket.currency.currency] &&
    currency_list[ticket.currency.currency].confirmations;
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withCoinsendaServices(TicketContainer))
);
