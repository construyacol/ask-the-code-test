...
class TicketPaymentProof extends Component {
  ...
  subirImg = async (img) => {
    ...
    let res = await this.props.coinsendaServices.confirmDepositOrder(
      ticket,
      base64
    );
  ...
  render() {
    ...
    const { fileloader, localImg } = this.state;

    // console.log('TicketPaymentProof', ticket)

    return (
      <div
        className={`TicketPaymentProof ${clases} `}
        style={{
          transform: fileloader ? "translateY(-50%)" : "translateY(0%)",
        }}
      >
        <div className="ctaPaymentProof fuente">
          ...
          <p className="titlePayment">
            <img src={imgTouch} alt="" width="70" />
            Toca para subir tu comprobante de pago
          </p>

          <p className="msgPayment">
            Sube una fotografía del comprobante de pago, si hiciste una
            transferencia por App, sube una captura de pantalla del registro del
            deposito.
          </p>
          {localImg && (
            <div>
              <img src={localImg} alt="" width="200" />
            </div>
          )}
        </div>
        ...
        <div className="droppedImgPaymentProof">
          {loader && (
            <div className="TloaderSec">
              <SimpleLoader label="Subiendo..." />
            </div>
          )}
          <CropImg
            {...this.state}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withCoinsendaServices(TicketPaymentProof));
