      ...
      <div className={`TicketDetail ${clases}`}>
        {!only_detail && (
          <div className="ticketHeader">
            ...
            type_order === "deposits" ? (
              ...
              <h1 className="fuente TicketTitle">
                Transacción{" "}
                {state === "pending"
                  ? "Pendiente"
                  : state === "accepted"
        ...
        <div className="contenidoTicket">
          {state === "confirmed" && currency_type === "fiat" && (
            ...
            <p className="TicketmsgPayment TicketConfirm fuente">
              Estamos a un paso de concretar tu retiro, muy pronto podrás
              disponer de tus fondos...
            </p>
          )}

          {state === "confirmed" && currency_type === "crypto" && (
            <p className="TicketmsgPayment TicketConfirm fuente">
              Genial, ahora solo queda tener algo de paciencia, esta ordén esta
              siendo confirmada, alcanzando {total_confirmations} confirmaciones
              en la blockchain pasará a "Aceptada" y podrás hacer uso de tu
              saldo.
            </p>
          )}
...
export const TicketPending = () => {
  return <img src={pending} alt="" width="73" />;
};
...
export const TicketCanceled = () => {
  return <img src={canceled} alt="" width="73" />;
};

export const TicketConfirmed = () => {
  return <img src={confirmed} alt="" width="73" />;
};
...
const selectWithMathItems = createSelector(
  ...
  (currencies, ticket) => {
    ...
    if (ticket.type_order === "swaps") {
      currency_type_bought = matchItem(
        currencies,
        { primary: ticket.currency_bought },
        "name"
      );
    }
