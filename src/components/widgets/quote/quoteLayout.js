         ...
         <LazyChart/>
          ...
          <QuoteLayoutContainer className="qLC">
            <PricesContainer className={`${movil ? 'movilPrices' : 'desktopPrices'} prices`}>
              ...
              <CoinList className={`coinList ${user_collection ? ((user_collection.length>1 && window.innerWidth>768) ? 'user_collection': '') : ''}`}>
                {
                  (user_collection) ?
                      (
                            ...
                            user_collection.map(item=>{
                              ...
                              return (
                                <Fragment key={item.id}>
                                  <ItemLayout actives={currentPair.primary_currency.currency === item.name && true } {...item} actualizarEstado={select_currency} />
                                </Fragment>
                              )
                            })
                        :
