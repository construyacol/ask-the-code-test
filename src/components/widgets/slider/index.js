  ...
  return(
    <SliderContainer onMouseOver={onOver}>
...

      {
        ...
        <Controls slider={slider} setcurrentPosition={setcurrentPosition} {...props}/>
      }
...
const Controls = ({ config:{ controls, autoSwap, swapInterval }, slider, setcurrentPosition }) => {
  ...
  const next = async(interval) =>{
    if(!interval){clearInterval(autoSwapInterval)}
  ...
  const actionHandle = (e) => {
    if(!slider.childNodes || !slider.childNodes.length){return false}
