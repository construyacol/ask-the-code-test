...
const KeyBoardContainer = ({ state }) => {
  return (
    <KeyBoard className={state}>
      ...
      <Key position="left" className="animated">
        {" "}
        <BsArrowLeft size={35} />
      </Key>
      <Key position="right" className="animated">
        {" "}
...
const KeyBoard = styled.div`
  ...
  column-gap: 10px;
  position: relative;
  -webkit-animation: ${keyBoardAnimation} 0.7s
    cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
  animation: ${keyBoardAnimation} 0.7s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
  &.getOut {
...
const Layout = styled.div`
  &.getOut {
    animation: ${getOutAnimation} 0.5s forwards;
    animation-delay: 1s;
  }
