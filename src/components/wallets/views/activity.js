...
const ActivityView = (props) => {
  ...
  const redirect = (activity_list) => {
    ...
    if (activity_list && activity_list.length) {
      return false;
    }
  ...
  const redirect_activity = async (path) => {
    ...
    return props.history.push(
      `/${params.primary_path}/activity/${params.account_id}/${path}`
    );
  };
  ...
  const get_activity = async (method) => {
    ...
    if (method !== "get_deposits") {
      if (
        ...
        activity_for_account[account_id].deposits &&
        activity_for_account[account_id].deposits.length
      ) {
        return redirect_activity("deposits");
      }
      if (
        !activity_for_account[account_id] ||
        (activity_for_account[account_id] &&
        ...
        if (activity.length) {
          return props.history.push(
            `/${params.primary_path}/activity/${params.account_id}/deposits`
          );
        }
      }
    }
    ...
    if (method !== "get_withdraws") {
      if (
        ...
        activity_for_account[account_id].withdraws.length
      ) {
        return redirect_activity("withdraws");
      }
      if (
        !activity_for_account[account_id] ||
        (activity_for_account[account_id] &&
        ...
        if (activity.length) {
          return props.history.push(
            `/${params.primary_path}/activity/${params.account_id}/withdraws`
          );
        }
      }
    }
    ...
    if (method !== "get_swaps") {
      if (
        activity_for_account[account_id] &&
        activity_for_account[account_id].swaps &&
        activity_for_account[account_id].swaps.length
      ) {
        return redirect_activity("swaps");
      }
      if (
        !activity_for_account[account_id] ||
        (activity_for_account[account_id] &&
        ...
        if (activity.length) {
          return props.history.push(
            `/${params.primary_path}/activity/${params.account_id}/swaps`
          );
        }
      }
    }
