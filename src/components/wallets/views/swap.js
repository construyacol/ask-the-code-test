...
function SwapView(props) {
  ...
  const idForClickeableElementPairSelector = useKeyActionAsClick(
    true,
    "show-pairs-button",
    112,
    false
  ...
  const { selectPair, isReady } = usePairSelector({
    ...props,
    actions,
    currentWallet,
    currencyPairs,
  ...
  useEffect(() => {
    ...
    coinsendaServices.getDefaultPair(
      currentWallet,
      local_currency,
      currentPair
    );
  ...
  const shouldActiveInput =
    active && secondary_coin && availableBalance > 0 && value > 0;
